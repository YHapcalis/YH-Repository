...
{
    /* 停机：两路都停，并把 compare 清 0 */
    if (s_htim == NULL)
    {
        return;
    }

    /* Stop + compare=0：双保险，确保两路都不输出 */
    HAL_TIM_PWM_Stop(s_htim, s_ch_fwd);
    HAL_TIM_PWM_Stop(s_htim, s_ch_rev);
    __HAL_TIM_SET_COMPARE(s_htim, s_ch_fwd, 0);
    __HAL_TIM_SET_COMPARE(s_htim, s_ch_rev, 0);
}
