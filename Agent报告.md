...
### Phase 8：仪表盘中文化 + OTA 备份进度实时显示（2026-07-03）

| 成果 | 详情 |
|------|------|
| 自定义中文字体 | `lv_font_conv` 生成 SimHei 16px 子集，仅含 ~50 界面汉字 + ASCII 全集 |
| 字体优化 | 替代内置 simsun_cjk（120KB 但缺字），新字体仅 ~7KB |
| 所有标签中文化 | 状态栏/传感器/系统/按钮全部中文 |
| °C/% 符号 | 自定义字体完整包含 |
| OTA 备份进度 | `lv_refr_now(NULL)` 实时显示，不触发 sysmon_timer |

### Phase 9：三屏扩展（Mode/设置/时钟 + 主题系统）（2026-07-04）

| 成果 | 详情 |
|------|------|
| Mode 屏 | 左侧系统信息 + 右侧硬件诊断（CAN 错误、复位源、外部 SRAM） |
| 设置屏 | 系统操作（重启/进入Bootloader/恢复出厂）+ 三套主题（暗黑/明亮/护眼） |
| 时钟屏 | `lv_calendar` 日历 + `lv_roller` 时/分滚轮选择 + CAN ID 0x14 时间同步到 F103 RTC |
| Home 屏重排 | 仪表盘中置(y=50)、指示灯居中、传感器一行20px底栏 |
| 主题系统 | `ui_apply_theme(theme_id)` 统一切换三屏背景色 |
| 屏切换动画 | `lv_scr_load_anim(FADE_IN, 200ms)` 全屏淡入 |
| 指针缓动 | `lv_anim` ease-out 120ms 平滑旋转 |
| GUI 栈 | 4KB → **8KB**（稳定） |
| LVGL 池 | 24KB → **32KB**（支持四屏控件） |

### Phase 10：SPI Flash 图片加载（2026-07-04）

| 成果 | 详情 |
|------|------|
| 图片搬迁 | 6 张图片（~52KB）从内部 Flash 移到 W25Q128 SPI Flash |
| 加载方式 | SPI Flash → 外部 SRAM (FSMC @ 0x68000000)，32 位逐字拷贝 |
| LVGL 渲染 | 直接从外部 SRAM 读取像素数据 |
| 加载器 | `spi_img_loader.c` — 启动时检测 SPI Flash，自动加载到外部 SRAM |
| 后备机制 | SPI Flash 数据不完整时从编译期图片自动补写 |
| 烧录路线 | 终端脚本 `make_img_batch.py` → `flash_new_batches.bat` → ST-LINK 批烧 |
| CAN 传输方案 | 放弃（CAN FIFO 溢出导致传输不可靠） |
| RAM 影响 | **无增加**（保留 48% 占用），32KB 临时缓冲已改为 256B 分块拷贝 |
| 内部 Flash 节省 | **~57KB**（图片数据由编译期移除） |

**关键修复：**
1. CAN FIFO 深度不足(3帧) → 数据帧大量丢失，放弃 CAN 传输路线
2. 外部 SRAM 32 位 FSMC 总线逐字节写入不可靠 → 改用内部缓冲 + 32 位字拷贝
3. 加载器入口检测首图地址(0x200) → 改为检测尾图地址(0xB800)，防部分写入误判
4. `flash_new_batches.bat` 遗留旧工程的 12 个 batch 文件覆盖数据 → 删除旧文件 + 改造脚本仅处理 batch_00
...
5. ~~**Mode 屏开发**~~ ✅ 已完成
6. ~~**LVGL 美化**~~ ✅ 已完成
7. ~~**SPI Flash 图片加载**~~ ✅ 已完成
8. **LittleFS 文件系统** — 在 W25Q128 上部署 LittleFS，统一管理文件（← 当前目标）
9. **双区 OTA 回滚** — 利用 SPI Flash 备份区做双区固件回滚
10. **外设扩展** — F103 和 F407 接入更多模块（GPS、气压计、陀螺仪等）
11. **固件签名验证** — CRC32/SHA256 + 签名校验增强安全性
