...
#include "custom.h"
// static lv_timer_t * task_chart;  // disabled: minimal mode screen
...
static void home_event_handler (lv_event_t *e)
{
    ...
    case LV_EVENT_SCREEN_LOADED:
    {
        ...
        /* home_label_digit_animation disabled — 速度数字固定不动 */
...
static void mode_event_handler (lv_event_t *e)
{
    ...
    case LV_EVENT_SCREEN_LOADED:
    {
        ...
        /* chart/music/nav widgets disabled in minimal mode — skip animations */
        break;
    }
    case LV_EVENT_SCREEN_UNLOADED:
    {
...
void events_init_mode (lv_ui *ui)
{
    ...
    if (ui->mode_label_prev)  lv_obj_add_event_cb(ui->mode_label_prev, mode_label_prev_event_handler, LV_EVENT_ALL, ui);
    if (ui->mode_label_play)  lv_obj_add_event_cb(ui->mode_label_play, mode_label_play_event_handler, LV_EVENT_ALL, ui);
    if (ui->mode_label_next)  lv_obj_add_event_cb(ui->mode_label_next, mode_label_next_event_handler, LV_EVENT_ALL, ui);
    if (ui->mode_img_home)    lv_obj_add_event_cb(ui->mode_img_home, mode_img_home_event_handler, LV_EVENT_ALL, ui);
}
