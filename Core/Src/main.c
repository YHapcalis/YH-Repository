...
/* USER CODE BEGIN PV */
volatile uint8_t  g_rx_byte = 0;   /* 接收缓冲 — ISR 写入, Task 消费      */
volatile uint8_t  g_rx_flag = 0;   /* 接收完成标志: 1=有新数据           */
/* USER CODE END PV */
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
    /* FreeRTOS 已接管 — LED 心跳 + UART 回显由 defaultTask 处理 */
    /* 此循环永不执行，仅作安全兜底                                    */
  /* USER CODE END 3 */
  }
}
